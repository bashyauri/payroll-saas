...
Route::middleware([
    ...
    Route::middleware(['auth', 'verified', EnsureBillingOnboardingComplete::class, EnforceReadOnlyMode::class])->group(function () {
        ...
        // Diagnostic route for debugging PAYE issues
        Route::get('debug/paye-emp1010', function () {
            ...
            return response()->json([
                'employee' => [
                    ...
                    'apply_paye_deduction' => $employee->apply_paye_deduction,
                    'apply_pension_deduction' => $employee->apply_pension_deduction,
                    'apply_nhf_deduction' => $employee->apply_nhf_deduction,
                    'apply_nhis_deduction' => $employee->apply_nhis_deduction,
                ],
                'settings' => [
                    'enabled_deductions' => $settings['enabled_deductions'] ?? [],
                    'paye_in_enabled' => in_array('paye', $settings['enabled_deductions'] ?? [], true),
                    'paye_consolidated_relief_percentage' => $settings['paye_consolidated_relief_percentage'] ?? null,
                    'paye_consolidated_relief_minimum' => $settings['paye_consolidated_relief_minimum'] ?? null,
                    'tax_brackets_count' => count($settings['paye_tax_brackets'] ?? []),
                ],
                'calculation' => [
                    'paye_deduction' => $calculation['paye_deduction'],
                    'pension_deduction' => $calculation['pension_deduction'],
                    'nhf_deduction' => $calculation['nhf_deduction'],
                    'nhis_deduction' => $calculation['nhis_deduction'],
                    'net_pay' => $calculation['net_pay'],
                ],
            ]);
        })->name('debug.paye.emp1010');
